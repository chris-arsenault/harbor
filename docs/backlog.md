...
- Thread previous-day high/low through the live, paper-forward, and edge-study session-level computation (and persist PDH/PDL on the sessions table) so every path sweeps the same prior-day levels the backtester now does.
